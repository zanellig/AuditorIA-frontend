...
export default function SettingsClient({
  ...
  // Use regular query instead of suspense query with initial data
  const { data: urlData } = useQuery({
    queryKey: ["apiUrls"],
    queryFn: async () => {
      const response = await fetch(`${host}/api/routes`)
      return response.json()
    },
    initialData: initialApiUrls, // Use the data passed from the server component
  })
              ...
              <CardContent className='space-y-4'>
                <div className='space-y-2'>
                  ...
                  <div className='flex flex-col lg:flex-row items-start justify-start gap-2'>
                    <Input
                      id='whisperApiUrl'
                      placeholder={urlData?.whisperUrl}
